...
def cli = new CliBuilder(usage: "WsGen [-h] [-d] [-j] [-c classpath] [-f wsdlfile] [-l url] [-p name] [-s name] [-t tns] interfaceName [--wsgen wsgen-opts ...]", posix: true);
cli.with {
    ...
    //n longOpt: 'name',            args: 1, argName: 'name',         'Name annotation attribute used in implementation class' // creates an error message
...
try {
  ...
  if (!fSuccess) {
    ...
    def pbArgs = [ "wsgen", "-r", temporaryFolder.getAbsolutePath(), "-d", temporaryFolder.getAbsolutePath(), "-cp", cpForProcessBuilder, implementationClassName ];
